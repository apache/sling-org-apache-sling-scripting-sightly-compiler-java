...
public final class JavaClassBackendCompiler implements BackendCompiler {
    ...
    static {
        try {
            mainTemplate = IOUtils.toString(
                    JavaClassTemplate.class.getClassLoader().getResourceAsStream(COMPILED_UNIT_TEMPLATE), "UTF-8");
            childTemplate = IOUtils.toString(
                    JavaClassTemplate.class.getClassLoader().getResourceAsStream(SUBTEMPLATE), "UTF-8");
        } catch (IOException e) {
    ...
    private void processCompilationResult(CompilationOutput result, JavaClassTemplate mainTemplate) {
        ...
        for (Map.Entry<String, CompilationOutput> entry :
                result.getSubTemplates().entrySet()) {
